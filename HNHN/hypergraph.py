...
class Hypertrain:
    ...
    def train(self, v, e, label_idx, labels):
        ...
        for i in range(self.args.n_epoch):
            ...
            # peed_all 是哪里来的
            # print(pred_all) # 所有的labels
            # pdb.set_trace()
            pred = pred_all[label_idx[i]]
            # print(label_idx.shape)
            # print(pred.shape)
            # pdb.set_trace()
            loss = self.loss_fn(pred, labels[i])
            test_err = self.eval(pred_all)
            if test_err < best_err:
                best_err = test_err
            sys.stdout.write('epoch {}:\t'.format(i)+' loss {} \n'.format(loss))
            # 记载loss值
            f = open("loss.txt", "a")
            # a =torch.Tensor()
    ...
    def eval(self, all_pred):
        ...
        if self.args.val_idx is None:
            ...
            ones = -torch.ones(len(all_pred[args.cur_epoch]))
            ones[self.args.val_idx[args.cur_epoch]] = 1
        ...
        f = open("acc.txt", "a")
        # a =torch.Tensor()
        f.write('epoch {}:'.format(args.cur_epoch) + str(acc) + '\n')
        f.close()
        if args.verbose:
...
def gen_data_cora(args, data_path='data/cora_author.pt', flip_edge_node=True, do_val=False):
    ...
    args.n_hidden = 512 if args.predict_edge else 400
    args.final_edge_dim = 100
    args.n_epoch = 200 if args.n_layers == 1 else 230  # 130 #120
    args.ne = n_author
    args.nv = n_paper
    # args.n_layers = 1 #2 #2
    ne = args.ne
    ...
    # print(args.all_labels.shape)  //775
    # pdb.set_trace()
    ...
    if proportional_select:
        ...
        for i in range(args.n_cls):
            ...
            rand_idx = torch.from_numpy(np.random.choice(len(cur_idx), size=(n_label_per_cls,), replace=False)).to(
                torch.int64)
            cur_idx = cur_idx[rand_idx]
            all_cls_idx.append(cur_idx)
        args.label_idx = torch.cat(all_cls_idx, 0)
    else:
        idx_train = idx_train.astype(float)
        args.label_idx = torch.from_numpy(idx_train).to(torch.int64) - 1
        # print(args.label_idx.shape)
        # print(args.label_idx.shape)
    ...
    if do_val:
        ...
        val_idx = torch.from_numpy(np.random.choice(len(args.label_idx), size=len(args.label_idx) // 5))
        val_idx = torch.tensor(val_idx).type(torch.long)
        idx_test = idx_test.astype(float)
        args.val_idx = torch.from_numpy(idx_test) - 1
        args.val_idx = args.label_idx[val_idx]
        ones = torch.ones(len(args.label_idx))
        # print(args.val_idx.shape)
        ones[val_idx] = -1
    ...
    print(args.label_idx.shape)
    # args.labels = np.zeros((args.label_idx))
    # print(args.label_idx.shape)
    print(args.all_labels.shape)
    pdb.set_trace()
    args.labels = args.label_idx
    for i in range(args.label_idx.shape[0]):
        m = torch.tensor(args.label_idx[i]).type(torch.long)
        # print(m.shape)
        # print(args.labels[i].shape)
        args.labels[i] = args.all_labels[m]
    # print(args.labels.shape)
    args.labels = args.labels.to(device)
...
def compare_normalization(data_path, args):
    ...
    for av, ae in a_list1:
        ...
        sys.stdout.write('\n ~~~Mean test err {}+-{} for alpha {} {} time {}~~~\n'.format(np.round(mean_err, 2),
                                                                                          np.round(err_std, 2),
                                                                                          args.alpha_v, args.alpha_e,
                                                                                          dur))
    ...
    print('best err {}+-{} best alpha_v {} alpha_e {} for dataset {}'.format(np.round(best_err * 100, 2),
                                                                             np.round(best_err_std * 100, 2),
                                                                             best_alpha_v, best_alpha_e,
                                                                             args.dataset_name))
    print('best ACC {}+-{} time {}+-{}'.format(np.round((1 - best_err) * 100, 2), np.round(best_err_std * 100, 2),
...
def select_params(data_path, args):
    ...
    for av, ae in a_list1:
        ...
        sys.stdout.write(
            '\n ~~~Mean VAL err {}+-{} for alpha {} {} time {}~~~\n'.format(np.round(mean_err, 2), np.round(err_std, 2),
                                                                            args.alpha_v, args.alpha_e, dur))
        if mean_err < best_err:
            best_err = mean_err
            best_err_std = err_std
            best_alpha_v = args.alpha_v
    ...
    print('best err {}+-{} best alpha_v {} alpha_e {} for dataset {}'.format(np.round(best_err * 100, 2),
                                                                             ...
                                                                             best_alpha_v, best_alpha_e,
                                                                             args.dataset_name))
    print('best validation ACC {}+-{} time {}+-{}'.format(np.round((1 - best_err) * 100, 2),
                                                          np.round(best_err_std * 100, 2), best_time, best_time_std))
...
if __name__ == '__main__':
    ...
    if dataset_name == 'cora':
        ...
        if args.do_svd:
            data_path = 'data/citeseer0_23703.pt'
        else:
            data_path = 'data/citeseer0_23703.pt'
